...
// This event handles adding/removing users from the role(s) they chose based on message reactions
        client.on('raw', async event => {
            ...
            if (!reaction) {
                ...
                //console.log(new Emoji(client, {animated: false, ...data.emoji}));
                reaction = new MessageReaction(client, emoji, message);
            }

            let embedFooterText;
            if (message.embeds[0]) embedFooterText = '';
            ...
            if (
                ...
                if (!CONFIG.embed && (message.embeds.length < 1)) {
                    ...
                    if (member.id !== client.user.id) {
                        ...
                        if (event.t === "MESSAGE_REACTION_ADD") await member.roles.add(guildRole.id);
                        else if (event.t === "MESSAGE_REACTION_REMOVE") await member.roles.remove(guildRole.id);
                    }
